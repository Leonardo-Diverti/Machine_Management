...
class MachineAdminDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'machine', 'tipo_documento', 'tipo_documento_display',
                  'display_name', 'numero_documento', 'data_documento', 'importo', 'fornitore',
    ...
    def get_display_name(self, obj):
        if obj.tipo_documento == 'ALTRO_ADMIN' and obj.descrizione:
            marker = 'Caricato dal beneficio fiscale ('
            if obj.descrizione.startswith(marker) and obj.descrizione.endswith(')'):
                return obj.descrizione[len(marker):-1]
...
class MachineStatusLogSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'machine', 'stato', 'timestamp']


class MachineFiscalBenefitDocumentSerializer(serializers.ModelSerializer):
    class Meta:
        model = MachineFiscalBenefitDocument
        fields = ['id', 'operation', 'file', 'uploaded_at', 'uploaded_by']
        read_only_fields = ['uploaded_at', 'uploaded_by']


class MachineFiscalBenefitMaintenanceDocumentSerializer(serializers.ModelSerializer):
    class Meta:
        model = MachineFiscalBenefitMaintenanceDocument
        fields = ['id', 'file', 'uploaded_at', 'uploaded_by']
        read_only_fields = ['uploaded_at', 'uploaded_by']


class MachineFiscalBenefitMaintenanceYearSerializer(serializers.ModelSerializer):
    documents = MachineFiscalBenefitMaintenanceDocumentSerializer(many=True, read_only=True)

    class Meta:
        model = MachineFiscalBenefitMaintenanceYear
        fields = ['id', 'anno', 'documents']


class MachineFiscalBenefitSerializer(serializers.ModelSerializer):
    documents = MachineFiscalBenefitDocumentSerializer(many=True, read_only=True)
    maintenance_years = MachineFiscalBenefitMaintenanceYearSerializer(many=True, read_only=True)

    class Meta:
        model = MachineFiscalBenefit
        fields = ['id', 'attivo', 'chiuso', 'documents', 'maintenance_years']
...
class MachineListSerializer(serializers.ModelSerializer):
    ...
    interconnessione_display = serializers.CharField(
        source='get_interconnessione_stato_display', read_only=True
    )
    fiscal_benefit = serializers.SerializerMethodField()

    class Meta:
        model = Machine
        fields = [
            'id', 'cdl', 'cc', 'capannone', 'stabilimento', 'tipo_macchina', 'commessa',
            'commessa_macchina', 'commessa_automazione', 'anno_avviamento',
            'stato', 'stato_display',
            'id_investimento_rm', 'id_investimento_consulente', 'consulente',
            'interconnessione_stato', 'interconnessione_display',
            'interconnessione_prevista', 'data_interconnessione_prevista',
            'created_at', 'updated_at', 'latest_status',
            'checklist_progress', 'fiscal_benefit'
        ]
    ...
    def get_fiscal_benefit(self, obj):
        if hasattr(obj, 'benefit_fiscal'):
            return MachineFiscalBenefitSerializer(obj.benefit_fiscal).data
        return None
...
class MachineDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Machine
        fields = ['id', 'cdl', 'cc', 'capannone', 'stabilimento', 'tipo_macchina', 'commessa',
              ...
              'latest_status', 'recent_logs', 'checklist_items', 'fiscal_benefit']
    ...
    def validate(self, attrs):
        ...
        # Rimuove campi fiscali in fase di creazione (possono essere inseriti solo da ADMIN successivamente)
        for field in ['id_investimento_rm', 'id_investimento_consulente', 'consulente']:
