...
def can_update_checklist_item(user, item):
    """Consente la modifica solo all'ufficio proprietario dell'attivita'."""
    if user.is_superuser:
        return True
    office_code = get_user_office_code(user)
    if item.solo_visualizzazione_tech and office_code == 'TECH':
        return False
    return office_code == item.ufficio
