...
* **Validazione Backend:** Le `views.py` e `serializers.py` di accounts interrogano il database (`db.sqlite3`) per verificare le credenziali e generano un "token" o una sessione sicura, gestita da `permissions.py` per definire cosa quell'utente può fare (es. ruolo di operatore vs. amministratore).
